...
class RandomForestClassifierCustom(BaseEstimator):
    ...
    def fit_tree(X, y):
        ...
        current_tree = current_tree_model.fit(X, y)
        #self.trees.append(current_tree)
        return current_tree

    def fit(self, X, y, n_jobs=1):
        '''
        Function to fit the random forest to a training set.
        '''
        ...
        for i in range(0, self.n_estimators):
            ...
            bootstrapped_pair = (bootstrapped_X_with_selected_features, bootstrapped_y)
            self.bootstrapped_data.append(bootstrapped_pair)
        
        with ProcessPoolExecutor(10) as pool:
            results = list(pool.map(self.fit_tree, self.bootstrapped_data))
        
        self.trees.append(results)
        return self
